	...
	if(suffix) {
		...
		if(suffix.indexOf("|")>-1) {
			...
			const info = msg.guild.roles.filter(role => {
				return role[1].id!=msg.guild.id && serverDocument.config.custom_roles.includes(role[1].id);
			}).sort((a, b) => {
				return b[1].position - a[1].position;
			}).map(role => {
				const count = memberArray.reduce((n, member) => {
					return n + member[1].roles.includes(role[1].id);
				}, 0);
				return `${role[1].name} (${count} member${count==1 ? "" : "s"})`;
			});
			if(info.length>0) {
