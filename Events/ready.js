...
module.exports = (bot, db, config, winston) => {
	...
	const runTimerExtensions = () => {
		const runTimerExtension = (svr, serverDocument, extensionDocument) => {
			winston.info(`Running timer extension '${extensionDocument.name}' in server '${svr.name}'`, {svrid: svr.id, extid: extensionDocument._id});
			for(let i=0; i<extensionDocument.enabled_channel_ids.length; i++) {
				const ch = svr.channels.get(extensionDocument.enabled_channel_ids[i]);
				if(ch) {
					runExtension(bot, db, winston, svr, serverDocument, ch, extensionDocument);
				}
			}
			setTimeout(() => {
				runTimerExtension(svr, serverDocument, extensionDocument);
					...
					if(svr) {
						serverDocuments[i].extensions.forEach(extensionDocument => {
							if(extensionDocument.type=="timer") {
								setTimeout(() => {
									runTimerExtension(svr, serverDocuments[i], extensionDocument);
